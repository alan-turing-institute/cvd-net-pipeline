...
def compute_pca(n_samples:int=50, n_params:int=9, n_pca_components:int=10, output_path:str="output"):
    ...
    if not os.path.exists(f"{output_path}/output{file_sufix}/pca"):
        os.makedirs(f"{output_path}/output{file_sufix}/pca")

    # Create directory for figures
    if not os.path.exists(f"{output_path}/output{file_sufix}/figures"):
        os.makedirs(f"{output_path}/output{file_sufix}/figures")
    ...
    plot_utils.plot_pca_explained_variance(pipeline, output_path=output_parameters)

    # Plot the PCA transformed data
    plot_utils.plot_pca_transformed(pipeline, X, output_path=output_parameters)
