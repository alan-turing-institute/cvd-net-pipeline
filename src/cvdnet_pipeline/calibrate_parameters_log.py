...
def calibrate_parameters_log(data_type="synthetic",
                         n_samples:int=64, 
                         n_params:int=9, 
    ...
    if data_type == "synthetic":
        ...
        input_params = pd.read_csv(f'{output_path}/pure_input{file_suffix}_log.csv')
        ...
        # emulators
        emulators = pd.read_pickle(f"{dir_name}/emulators/linear_models_and_r2_scores_{n_samples}_log.pkl")
    ...
    if data_type == "synthetic":
        ...
        # Expoentiate samples to get back to original scale * ref
        samples_df_exp = pd.DataFrame(np.exp(bc.samples_df), columns=bc.param_names)
        samples_df_exp.to_csv(f"{output_dir_bayesian}/posterior_samples_exp.csv", index=False)
    ...
    return output_dir_bayesian, e_obs, samples_df_exp,