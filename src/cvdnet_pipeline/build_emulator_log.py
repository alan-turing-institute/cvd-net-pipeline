...
def build_emulator_log(n_samples:int=200, 
                   n_params:int=9, 
    ...
    # Take log of inputs and divide by sample mean to normalise
    filtered_input_mean = filtered_input.mean()
    log_filtered_input = np.log(filtered_input / filtered_input_mean)
    ...
    for key in output_keys:
        model, r2, mse, rmse, rse = helper_functions.emulate_linear(input=log_filtered_input, output=output_file[key])
    ...
    emulator_results_df.to_csv(f'{output_path}/output{file_suffix}/emulators/linear_models_and_r2_scores_{n_samples}_log.csv')

    # To save the DataFrame with models, use pickle
    emulator_results_df.to_pickle(f'{output_path}/output{file_suffix}/emulators/linear_models_and_r2_scores_{n_samples}_log.pkl')

    # Save log of input parameters
    log_input_df = pd.DataFrame(log_filtered_input)
    log_input_df.to_csv(f"{output_path}/pure_input{file_suffix}_log.csv", index=False)

    # Save mean of input parameters (used for normalisation)    
    filtered_input_mean = pd.DataFrame(filtered_input_mean).T
    filtered_input_mean.to_csv(f"{output_path}/pure_input{file_suffix}_mean.csv", index=False)
    ...
    if output_keys_red is not None:
        ...
        reduced_emulator_results_df.to_csv(f'{output_path}/output{file_suffix}/emulators/calibration_features_linear_models_and_r2_scores_{n_samples}_log.csv')
    
    return filtered_input_mean